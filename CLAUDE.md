...
- Overdue visual treatment: red left stripe on card left edge, red checkbox border, red bold date, section "N overdue" count badges, ⚠ Overdue filter pill — tasks never displace from original sections ✓
- Green bold "Today" text for tasks due today ✓
- Filter bar: single clean row — source pills left, status pills middle, Overdue+Filter+Group right ✓
